...
herokuForcePush = ({ app_name }) => {
  const push = execSync(
    `heroku container:push --force --app ${app_name} web`,
    execOptions
  ).toString();
  console.log(push);
};
...
deployWithBuildManifest = () => {
  try {
    ...
    const status = execSync("git status --porcelain", execOptions)
      .toString()
      .trim(); //checking if there is a modified file
    console.log(status);
    if (status) {
...
pushAndRelease = ({ use_docker, use_git, use_build_manifest }) => {
  try {
    ...
    if (use_docker === "true") {
      ...
      (use_docker && use_git) === "true" ||
      (use_build_manifest && use_git) === "true" ||
      (use_build_manifest && use_docker && use_git) === "true"
    ) {
      ...
      console.log(
        "No deployment method is specified. Attempting to use default use git..."
      );
      deployWithGit();
    }
  } catch (error) {
...
core.setOutput("status", "Successfully deployed app from branch");
